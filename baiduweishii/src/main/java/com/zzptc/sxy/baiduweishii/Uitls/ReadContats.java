...
import com.zzptc.sxy.baiduweishii.contant.MobileNumberDemo;
 ...
 */
public class ReadContats {

    public static List<Contant> contacts;

    public static List<Contant> getContacts() {
        return contacts;
    }

    public static void setContacts(List<Contant> contacts) {
        ReadContats.contacts = contacts;
    }
    ...
    //创建类  Contact
    public static List<Contant> queryAllContacts(Context context){
        List<Contant> contacts = new ArrayList<>();
    ...
    private static String getPhoneAttribute(String phone){
        ...
        if(phone.matches("^1[34578]\\d{9}$")){
            try {
                MobileNumberDemo mobileAttribute = dbManager.selector(MobileNumberDemo.class).where("MobileNumber","=",phone.substring(0,7)).findFirst();
